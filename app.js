...
function createLadder(preferredHyeBinTeam){for(let attempt=0;attempt<200;attempt+=1){const rungs=[];for(let row=0;row<ladderRowCount;row+=1){const rowRungs=[];for(const col of shuffle([0,1,2,3,4])){if(Math.random()<.56&&!rowRungs.includes(col-1)&&!rowRungs.includes(col+1))rowRungs.push(col);}if(!rowRungs.length)rowRungs.push(Math.floor(Math.random()*5));rowRungs.sort((a,b)=>a-b);rungs.push(rowRungs);}const paths=ladderPlayers.map((_,index)=>traceLadder(index,rungs));let endpointTeams=shuffle(['1','1','1','2','2','2']);let results=paths.map(path=>endpointTeams[path.endLane]);if(results[4]===results[5])continue;if(results[4]!==String(preferredHyeBinTeam)){endpointTeams=endpointTeams.map(team=>team==='1'?'2':'1');results=paths.map(path=>endpointTeams[path.endLane]);}return {rungs,endpointTeams,paths,results};}throw new Error('사다리 경로를 만들지 못했습니다. 다시 시도해 주세요.');}
function drawLadder(){const canvas=$('ladderCanvas'),ctx=canvas.getContext('2d'),h=canvas.height,bottom=h-45;ctx.clearRect(0,0,canvas.width,h);ctx.strokeStyle='#c9d2e6';ctx.lineWidth=2;ctx.setLineDash([4,5]);ladderXs.forEach(x=>{ctx.beginPath();ctx.moveTo(x,ladderTop);ctx.lineTo(x,bottom);ctx.stroke();});ctx.setLineDash([]);if(!currentAssignment)return;ctx.strokeStyle='#7d95de';ctx.lineWidth=3;for(let row=0;row<currentAssignment.rungs.length;row+=1){const y=ladderRowY(row,h);for(const col of currentAssignment.rungs[row]){ctx.beginPath();ctx.moveTo(ladderXs[col],y);ctx.lineTo(ladderXs[col+1],y);ctx.stroke();}}for(const path of currentAssignment.paths){let lane=path.start;ctx.beginPath();ctx.moveTo(ladderXs[lane],ladderTop);for(let row=0;row<ladderRowCount;row+=1){const y=ladderRowY(row,h);const nextLane=path.lanes[row+1];ctx.lineTo(ladderXs[lane],y);if(nextLane!==lane)ctx.lineTo(ladderXs[nextLane],y);lane=nextLane;}ctx.lineTo(ladderXs[lane],bottom);ctx.strokeStyle=currentAssignment.results[path.start]==='1'?'rgba(40,87,238,.56)':'rgba(243,72,114,.56)';ctx.lineWidth=5;ctx.lineCap='round';ctx.stroke();ctx.fillStyle=currentAssignment.results[path.start]==='1'?'#2857ee':'#f34872';ctx.beginPath();ctx.arc(ladderXs[lane],bottom,5,0,Math.PI*2);ctx.fill();}}
function renderTargets(){const targetTeams=currentAssignment?.endpointTeams||['1','1','1','2','2','2'];$('ladderTargets').innerHTML=targetTeams.map(team=>`<span class="${team==='1'?'team-one':'team-two'}">${team} TEAM</span>`).join('');}
function renderLadder(){$('ladderPlayers').innerHTML=ladderPlayers.map(p=>`<span>${p}</span>`).join('');renderTargets();drawLadder();if(!currentAssignment){$('assignmentEmpty').hidden=false;$('assignmentResult').hidden=true;return;}$('assignmentEmpty').hidden=true;const result=$('assignmentResult');result.hidden=false;result.innerHTML=teamCard(1,currentAssignment.team1)+teamCard(2,currentAssignment.team2);}
...
function assignTeams(){if(!ready||working)return;enqueue(base=>{const ladder=createLadder(base.nextHyeBinTeam);const team1=[],team2=[];ladder.results.forEach((team,index)=>(team==='1'?team1:team2).push(ladderPlayers[index]));currentAssignment={...ladder,team1,team2};return {...base,nextHyeBinTeam:base.nextHyeBinTeam===1?2:1};},'실제 사다리 경로로 팀이 정해졌어요!').then(ok=>{if(ok)renderLadder();else{currentAssignment=null;renderLadder();}});}
